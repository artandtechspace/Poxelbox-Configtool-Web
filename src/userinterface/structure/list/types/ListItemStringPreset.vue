...
import type { IntPresetValue, IntValue, StringPresetValue } from '@/schema/DataTypes';
...
export default {
    ...
    props: {
        ...
        itm: {
            required: true,
            type: Object as PropType<StringPresetValue>
        }
    }
}
</script>
...
<template>
    <ListItemWrapper :itm="itm" :name="name">
        <template #default>
            <select v-model="itm.value">
                <option :key="id" v-for="opt, id in itm.presets" :value="opt">{{ opt }}</option>
            </select>
            <Spacer />
        </template>
    </ListItemWrapper>
